...
export const useVotingStore = create<VotingState>((set, get) => ({
  ...
  fetchVotesByEvent: async (eventId: number, params?: Record<string, any>) => {
    ...
    try {
      const response: PaginatedVotesResponse = await apiListVotes({
        ...params,
        event: eventId,
      })
      set({
      ...
      const errorMessage =
        error.response?.data?.detail || error.message || 'Failed to fetch votes'
  ...
  createVote: async (voteData: CreateVoteData, eventId: number) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.detail || error.message || 'Failed to create vote'
      set({ isLoading: false, error: errorMessage })
      return null
    }
  },
  ...
  castVote: async (voteId: number, optionsData: CastVoteData) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.detail || error.message || 'Failed to cast vote'
      set({ isLoading: false, error: errorMessage })
      return false
    }
  },
  ...
  handleVoteUpdated: (updatedVote: Vote) => {
    // E.g. when a vote is cast, or status changes
    set(state => ({
      votes: state.votes.map(vote =>
        vote.id === updatedVote.id ? updatedVote : vote
      ),
      currentVote:
        state.currentVote?.id === updatedVote.id
          ? updatedVote
          : state.currentVote,
    }))
  ...
  handleVoteResultsUpdated: (results: VoteResults) => {
    ...
    set(state => ({
      ...
      currentVote:
        state.currentVote?.id === results.id
          ? {
              ...state.currentVote,
              total_votes: results.total_votes,
              options: results.results.map(r => ({
  ...
  handleVoteDeleted: (voteId: number) => {
    ...
    set(state => ({
      ...
      currentVoteResults:
        state.currentVoteResults?.id === voteId
          ? null
          : state.currentVoteResults,
    }))
  },
}))
