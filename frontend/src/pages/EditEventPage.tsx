...
import { UpdateEventData, Event } from '../services/eventService';
...
const EditEventPage: React.FC = () => {
  ...
  if (isLoading && isAuthorized === null) { // Show loading only on initial fetch and auth check
     ...
     return (
        <div className="container mx-auto py-8 text-center">
            <Card className="w-full max-w-md mx-auto glass-card p-8">
                <CardTitle className="text-2xl text-red-400">Not Authorized</CardTitle>
                <CardDescription className="text-gray-300 mt-2">
                    You do not have permission to edit this event.
                </CardDescription>
                <Button onClick={() => navigate(eventId ? `/events/${eventId}` : '/events')} className="mt-6 btn-aurora">
                    Back to Event
                </Button>
            </Card>
        </div>
  ...
  if (!currentEvent && !isLoading) { // Event not found after loading attempt
    return <div className="text-center py-10 text-red-400">Event not found or could not be loaded.</div>;
  }
      ...
      <Card className="w-full max-w-2xl mx-auto glass-card">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {error && !formError && <p className="text-sm text-red-400 text-center">Store error: {error}</p>}


            <div className="flex justify-end gap-4 pt-4">
              <Button type="button" variant="outline" onClick={() => navigate(eventId ? `/events/${eventId}` : '/events')} className="btn-glass" disabled={isLoading}>
                Cancel
              </Button>
