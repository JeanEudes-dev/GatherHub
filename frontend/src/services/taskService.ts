...
export const listTasks = async (params?: Record<string, any>): Promise<PaginatedTasksResponse> => {
...
export const assignTask = async (taskId: number, assigneeId: number): Promise<{ message: string; task: Partial<Task> }> => {
  const response = await apiClient.post(`/tasks/${taskId}/assign/`, { assignee: assigneeId });
  return response.data;
};

export const completeTask = async (taskId: number): Promise<{ message: string; task: Partial<Task> }> => {
