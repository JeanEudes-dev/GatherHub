...
interface TaskState {
  ...
  isLoading: boolean;
  error: string | null;
  pagination: { // If listing tasks globally with pagination
    count: number;
    next: string | null;
  ...
  // Actions
  fetchTasksByEvent: (eventId: number, params?: Record<string, any>) => Promise<void>; // Most common use case
  fetchTaskById: (taskId: number) => Promise<Task | null>;
  createTask: (taskData: CreateTaskData, eventId: number) => Promise<Task | null>; // eventId to update specific event's tasks
  updateTask: (taskId: number, taskData: UpdateTaskData, eventId?: number) => Promise<Task | null>;
  deleteTask: (taskId: number, eventId?: number) => Promise<boolean>;
  assignTask: (taskId: number, assigneeId: number, eventId?: number) => Promise<boolean>;
...
export const useTaskStore = create<TaskState>((set, get) => ({
  ...
  fetchTasksByEvent: async (eventId: number, params?: Record<string, any>) => {
    ...
    try {
      ...
      const response: PaginatedTasksResponse = await apiListTasks({ ...params, event: eventId });
      set({
  ...
  fetchTaskById: async (taskId: number) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.detail || error.message || 'Failed to fetch task details';
  ...
  createTask: async (taskData: CreateTaskData, eventId: number) => {
    ...
    try {
      ...
      if (taskData.event === eventId) { // Ensure it's the correct event context
  ...
  updateTask: async (taskId: number, taskData: UpdateTaskData, eventId?: number) => {
    set({ isLoading: true, error: null });
    try {
  ...
  deleteTask: async (taskId: number, eventId?: number) => {
    set({ isLoading: true, error: null });
    try {
    ...
    } catch (error: any)
 {
  ...
  assignTask: async (taskId: number, assigneeId: number, eventId?: number) => {
    set({ isLoading: true, error: null });
    try {
      const response = await apiAssignTask(taskId, assigneeId);
      // Refetch task or update optimistically
      const updatedTaskDetails = await apiGetTaskDetails(taskId);
      set(state => ({
        tasks: state.tasks.map(task => task.id === taskId ? updatedTaskDetails : task),
        currentTask: state.currentTask?.id === taskId ? updatedTaskDetails : state.currentTask,
        isLoading: false,
      }));
      return true;
    } catch (error: any) {
  ...
  completeTask: async (taskId: number, eventId?: number) => {
    set({ isLoading: true, error: null });
    try {
  ...
  handleTaskDeleted: (taskId: number, eventId?: number) => {
    set(state => ({
      tasks: state.tasks.filter(task => task.id !== taskId),
      currentTask: state.currentTask?.id === taskId ? null : state.currentTask,
    }));
