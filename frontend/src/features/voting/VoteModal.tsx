...
const VoteModal: React.FC<VoteModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && voteToDisplay) {
      ...
      // Reset local state
      setSelectedOptions(
        voteToDisplay.user_vote?.option_id
          ? [voteToDisplay.user_vote.option_id]
          : []
      ) // Pre-select if already voted (single choice example)
  ...
  const handleCastVote = async () => {
    ...
    setLocalError(null)
    const success = await castVote(voteToDisplay.id, {
  ...
  const canVote =
    isAuthenticated() &&
    voteToDisplay &&
    voteToDisplay.status === 'active' &&
    !voteToDisplay.user_voted
  const showResults =
    voteToDisplay &&
    (voteToDisplay.status === 'ended' ||
      (voteToDisplay.user_voted && voteToDisplay.status === 'active'))

  if (!isOpen || !voteToDisplay) return null

  return (
    <Dialog open={isOpen} onOpenChange={open => !open && onClose()}>
      <DialogContent className="glass-card flex max-h-[85vh] flex-col border-gray-700 text-white sm:max-w-lg md:max-w-xl">
        <DialogHeader className="flex-shrink-0">
          ...
          <DialogDescription className="text-gray-400">
            ...
            <div className="mt-1 text-xs">
              Status:{' '}
              <span
                className={`font-semibold capitalize ${voteToDisplay.status === 'active' ? 'text-green-400' : 'text-gray-500'}`}
              >
                {voteToDisplay.status}
              </span>
              {voteToDisplay.ends_at && (
                <span> | Ends: {formatDate(voteToDisplay.ends_at)}</span>
              )}
            </div>
          </DialogDescription>
        </DialogHeader>

        <div className="flex-grow space-y-6 overflow-y-auto py-4 pr-2">
          {/* Voting Section or Message */}
          {!showResults && canVote && (
            <div className="space-y-3">
              <h3 className="text-md mb-1 font-semibold text-gray-200">
                Select your choice{voteToDisplay.multiple_choice ? '(s)' : ''}:
              </h3>
              {voteToDisplay.options.map(option => (
                <div
                  key={option.id}
                  className={`flex items-center space-x-3 rounded-md p-3 transition-all ${selectedOptions.includes(option.id!) ? 'bg-aurora-purple/30 ring-2 ring-aurora-purple' : 'bg-white/10 hover:bg-white/20'}`}
                >
                  {voteToDisplay.multiple_choice ? (
                    ...
                    >
                      <RadioGroupItem
                        ...
                        id={`option-${option.id}`}
                        className="border-gray-500 text-aurora-purple ring-aurora-purple focus:ring-aurora-purple data-[state=checked]:border-aurora-purple"
                      />
                    </RadioGroup>
                  )}
                  <Label
                    htmlFor={`option-${option.id}`}
                    className="flex-grow cursor-pointer text-sm font-medium text-gray-100"
                  >
                    {option.text}
                  </Label>
                </div>
            ...
            !showResults && (
              <div className="rounded-md bg-green-500/10 p-4 text-center">
                ...
                <p className="text-xs text-gray-400">
                  Results will be shown once the poll ends or if the creator
                  shares them.
                </p>
              </div>
            )}
          {voteToDisplay.status === 'ended' &&
            !showResults && ( // Should typically show results if ended
              <div className="rounded-md bg-gray-500/10 p-4 text-center">
          ...
          {showResults && currentVoteResults && (
            <div className="space-y-3">
              <h3 className="text-md flex items-center font-semibold text-gray-200">
                <BarChart2 size={18} className="mr-2 text-aurora-blue" />
                Vote Results ({currentVoteResults.total_votes} total votes)
              </h3>
              {currentVoteResults.results
                ...
                .map(option => (
                  <div key={option.id} className="space-y-1">
                    ...
                    <Progress
                      ...
                      className="h-2 [&>*]:bg-gradient-to-r [&>*]:from-aurora-blue [&>*]:to-aurora-purple"
                    />
                  </div>
                ))}
              {currentVoteResults.winner && (
                <p className="pt-3 text-center text-lg font-semibold text-aurora-green">
                  Winner: {currentVoteResults.winner.text}
                </p>
              )}
            </div>
          )}
          {isStoreLoading && (showResults || canVote) && (
        ...
        <DialogFooter className="flex-shrink-0 pt-4">
          {localError && (
            <p className="flex-grow text-left text-sm text-red-400">
              {localError}
            </p>
          )}
          ...
          <DialogClose asChild>
            <Button type="button" variant="outline" className="btn-glass">
              Close
            </Button>
          </DialogClose>
          {canVote && (
