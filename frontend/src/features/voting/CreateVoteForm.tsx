...
import type { CreateVoteData } from '../../services/votingService'
...
const CreateVoteForm: React.FC<CreateVoteFormProps> = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={open => !open && onClose()}>
      <DialogContent className="glass-card border-gray-700 text-white sm:max-w-lg">
        <DialogHeader>
        ...
        <form
          ...
          className="max-h-[70vh] space-y-4 overflow-y-auto py-4 pr-2"
        >
          <div>
            <Label htmlFor="voteTitle" className="text-gray-300">
              Vote Title
            </Label>
            <Input
              id="voteTitle"
              ...
              required
              className="border-white/20 bg-white/10 placeholder-gray-500"
            />
          </div>
          <div>
            <Label htmlFor="voteDescription" className="text-gray-300">
            ...
            {options.map((option, index) => (
              <div key={index} className="flex items-center gap-2">
                <Input
                  ...
                  required
                  className="flex-grow border-white/20 bg-white/10 placeholder-gray-500"
                />
                {options.length > 2 && (
                  <Button
                    type="button"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="endsAtDate" className="text-gray-300">
                Ends On (Optional Date)
              </Label>
              <Input
                id="endsAtDate"
                type="date"
            ...
            <div>
              <Label htmlFor="endsAtTime" className="text-gray-300">
                Ends At (Optional Time)
              </Label>
              <Input
                id="endsAtTime"
                type="time"
          ...
          <div className="flex items-center space-x-2 pt-2">
            <Checkbox
              ...
              onCheckedChange={checked => setMultipleChoice(Boolean(checked))}
              className="border-gray-500 data-[state=checked]:bg-aurora-purple"
            />
          ...
          {formError && (
            <p className="pt-2 text-center text-sm text-red-400">{formError}</p>
          )}
