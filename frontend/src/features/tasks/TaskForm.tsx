...
const TaskForm: React.FC<TaskFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      ...
      setFormError(
        error || `Failed to ${taskToEdit ? 'update' : 'create'} task.`
      )
  ...
  return (
    <Dialog open={isOpen} onOpenChange={open => !open && onClose()}>
      <DialogContent className="glass-card border-gray-700 text-white sm:max-w-[525px]">
        <DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                <SelectTrigger className="border-white/20 bg-white/10">
          ...
          <div>
            <Label htmlFor="assigneeId" className="text-gray-300">
              Assign To
            </Label>
            <Select
              value={assigneeId}
              onValueChange={v => setAssigneeId(v === '' ? undefined : v)}
            >
              <SelectTrigger className="border-white/20 bg-white/10">
