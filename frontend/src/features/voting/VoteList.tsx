...
const VoteList: React.FC<VoteListProps> = ({ eventId, isEventCreator }) => {
  ...
  if (isLoading && votes.length === 0) {
    return <div className="text-center py-6 text-gray-400">Loading votes...</div>;
  }

  if (error) {
    return <div className="text-center py-6 text-red-400">Error loading votes: {error}</div>;
  }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        {isEventCreator && isAuthenticated && (
          <Button onClick={handleOpenCreateVoteForm} className="btn-glass text-sm">
      ...
      {eventSpecificVotes.length === 0 && !isLoading && (
        <div className="glass-card p-6 rounded-md text-center">
          <p className="text-gray-400">No votes or polls have been created for this event yet.</p>
          {isEventCreator && isAuthenticated && (
            <Button onClick={handleOpenCreateVoteForm} variant="link" className="text-aurora-blue mt-2">
